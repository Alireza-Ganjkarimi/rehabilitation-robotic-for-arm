...
def vision_worker_process(vision_queue, flexion_queue, running_event):
    ...
    # Initialize camera and vision tracker within the isolated process
...
def robot_worker_process(flexion_queue, robot_queue, running_event):
    ...
    # Initialize the controller and simulator within the isolated process
    controller = FingerAANController(effort_threshold=0.25, intent_threshold=0.1)
    ...
    while running_event.is_set():
        ...
        # Drain the queue to ensure we process the most recent sensor data
        ...
        if current_flexion is not None:
            ...
            # Throttle render rate to ~30 FPS to maintain simulation performance
        ...
        # Run physics calculations dynamically at exactly ~60 Hz
...
class RehabDashboard:
    def __init__(self, root):
        ...
        # Robot force assistance visual indicators
        ...
        # Native canvas used for high-performance, real-time plotting
        ...
        # --- Multiprocessing Variables and Queues ---
        self.running_event = mp.Event()
        self.running_event.set()
        
        # Inter-process communication queues (size limited to prevent latency)
    ...
    def draw_canvas_chart(self):
        """Renders the real-time alpha value history using native canvas operations."""
        ...
        for i, val in enumerate(self.alpha_history):
            x = margin_x + (i * dx)
            # Map normalized [0, 1] value to canvas coordinates
    ...
    def update_ui_loop(self):
        """Non-blocking UI loop for fetching queue data and updating widgets."""
        # Fetch the latest camera frame if available
        try:
        ...
        # Fetch the latest simulation frame if available
        try:
        ...
        # Update UI state and indicators
        ...
        # Schedule the next UI update (targeting ~60 FPS)
        self.root.after(16, self.update_ui_loop)


    def on_close(self):
        """Safely terminates worker processes on application exit."""
        self.running_event.clear()
        
        # Allow processes time to shut down gracefully
        self.vision_process.join(timeout=1.0)
        self.robot_process.join(timeout=1.0)
        
        # Force terminate if processes hang
...
if __name__ == "__main__":
    # Required for safe multiprocessing execution on Windows environments
    mp.freeze_support()
